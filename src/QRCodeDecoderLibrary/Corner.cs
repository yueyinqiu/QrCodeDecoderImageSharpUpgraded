...
internal sealed class Corner
{
    ...
    private Corner(Finder TopLeftFinder, Finder TopRightFinder, Finder BottomLeftFinder)
    {
        ...
        // top line slope
        this.TopLineDeltaX = TopRightFinder.Col - TopLeftFinder.Col;
        this.TopLineDeltaY = TopRightFinder.Row - TopLeftFinder.Row;

        // top line length
        this.TopLineLength = Math.Sqrt(this.TopLineDeltaX * this.TopLineDeltaX + this.TopLineDeltaY * this.TopLineDeltaY);

        // left line slope
        this.LeftLineDeltaX = BottomLeftFinder.Col - TopLeftFinder.Col;
        this.LeftLineDeltaY = BottomLeftFinder.Row - TopLeftFinder.Row;

        // left line length
        this.LeftLineLength = Math.Sqrt(this.LeftLineDeltaX * this.LeftLineDeltaX + this.LeftLineDeltaY * this.LeftLineDeltaY);
        return;
    }
    ...
    internal int InitialVersionNumber()
    {
        ...
        if (Math.Abs(this.TopLineDeltaX) >= Math.Abs(this.TopLineDeltaY))
        {
            TopModules += this.TopLineLength * this.TopLineLength /
                (Math.Abs(this.TopLineDeltaX) * 0.5 * (this.TopLeftFinder.HModule + this.TopRightFinder.HModule));
        }
        ...
        {
            TopModules += this.TopLineLength * this.TopLineLength /
                (Math.Abs(this.TopLineDeltaY) * 0.5 * (this.TopLeftFinder.VModule + this.TopRightFinder.VModule));
        }
        ...
        if (Math.Abs(this.LeftLineDeltaY) >= Math.Abs(this.LeftLineDeltaX))
        {
            LeftModules += this.LeftLineLength * this.LeftLineLength /
                (Math.Abs(this.LeftLineDeltaY) * 0.5 * (this.TopLeftFinder.VModule + this.BottomLeftFinder.VModule));
        }
        ...
        {
            LeftModules += this.LeftLineLength * this.LeftLineLength /
                (Math.Abs(this.LeftLineDeltaX) * 0.5 * (this.TopLeftFinder.HModule + this.BottomLeftFinder.HModule));
        }
